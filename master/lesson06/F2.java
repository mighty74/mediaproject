...
public class F2 {
	public static void main(String[] args)
	{
	     ...
	     for(int i = 0; i < height; i++) {
	    	 for(int l = 0; l < width; l++) {
	    		 if(l > 0 && l < width-1 && i > 0 && i < height-1) {
	    			 int num = img0.pixel[i-1][l+1] + 2*img0.pixel[i][l+1] + img0.pixel[i+1][l+1]
	    					 - img0.pixel[i-1][l-1] - (2*img0.pixel[i][l-1]) - img0.pixel[i+1][l-1];
	    			 if(num > 255) {
	    				 num = 255;
	    			 }
	    			 if(num < 0) {
	     ...
	     /**
	      *  Set the file name to "My_ImageF2".
	      */
	     String fileName01 = "bmp/My_ImageF2";
